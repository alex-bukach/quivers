...
use Drupal\quivers_drupal_plugin\ErrorHelper;
 ...
 *   forms = {
 *     "add-payment-method" = "Drupal\quivers_drupal_plugin\PluginForm\Stripe\PaymentMethodAddForm",
 *   },
 *   payment_method_types = {"credit_card"},
 *   credit_card_types = {
 *     "amex", "dinersclub", "discover", "jcb", "maestro", "mastercard", "visa",
 *   },
 *   js_library = "quivers_drupal_plugin/form",
 * )
 */
class Stripe extends OnsitePaymentGatewayBase implements StripeInterface {
  ...
  public function createPayment(PaymentInterface $payment, $capture = FALSE) {
    ...
    $transaction_data = [
      ...
      'capture' => false,
      'description' => $payment->getOrder()->getEmail(),
    ];
    ...
    // Adding customer-info to the Charge.
    $transaction_data['shipping'] = array(
      "address" => array(
        'line1' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getAddressLine1(),
        'line2' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getAddressLine2(),
        'city' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getLocality(),
        'country' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getCountryCode(),
        'postal_code' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getPostalCode(),
        'state' => $payment->getOrder()->getBillingProfile()->get('address')->first()->getAdministrativeArea(),
      ),
      "name" => sprintf('%s %s', 
        $payment->getOrder()->getBillingProfile()->get('address')->first()->getGivenName(), 
        $payment->getOrder()->getBillingProfile()->get('address')->first()->getFamilyName()),
    );
    ...
    $next_state = false ? 'completed' : 'authorization';
